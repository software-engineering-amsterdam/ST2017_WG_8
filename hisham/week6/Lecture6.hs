...
-- Added the solutions for Lab6 at the bottom of this file
...
exM x y n =
    let
        (a,b) = 
            let
                v = last (takeWhile (< y) (iterate (2*) 1))
            in (v, rem y v)
        r = exM x a n
    in
        if b > 0 then (r * exM x b n) `mod` n
                 else (r * r) `mod` n
...
test4 :: Int -> IO ()
test4 k = test k composites
    where
    test k' (x:xs) = do
        result <- primeTestsF k' x
        if result then 
            print ("Prime testing with k= " ++ show k' ++ " failed on: " ++ show x) 
        else test k' xs
...
-- The absolute minimum that is found: 294409