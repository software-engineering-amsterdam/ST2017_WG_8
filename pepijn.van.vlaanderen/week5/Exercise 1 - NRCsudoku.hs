...
-- For the NRC sudoku add those new blocks
blocks' :: [[Int]]
blocks' = [[2..4],[6..8]]
...
-- Check if element x is in a NRC block, same as bl for normal Sudoku blocks.
bl' :: Int -> [Int]
bl' x = concat $ filter (elem x) blocks'
...
-- Create a list with the current values for a NRC block
subGrid' :: Sudoku -> (Row,Column) -> [Value]
subGrid' s (r,c) =
  [ s (r',c') | r' <- bl' r, c' <- bl' c ]
...
-- Check which elements are free in a NRC subgrid.
freeInSubgrid' :: Sudoku -> (Row,Column) -> [Value]
freeInSubgrid' s (r,c) = freeInSeq (subGrid' s (r,c))

-- Add check if element is also free in a NRC subgrid.
...
-- Check if a NRC subgrid doesn't contain duplicate values.
subgridInjective' :: Sudoku -> (Row,Column) -> Bool
subgridInjective' s (r,c) = injective vs where
   vs = filter (/= 0) (subGrid' s (r,c))

-- Add check for NRC subgrid that it doesn't contain duplicate values, otherwise the Sudoku is not consistent.
...
-- Check if it is the same NRC block
sameblock' :: (Row,Column) -> (Row,Column) -> Bool
sameblock' (r,c) (x,y) = bl' r == bl' x && bl' c == bl' y
